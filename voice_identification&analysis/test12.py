...
def on_button_click(action, sub_frame, name_label):
    if action == "açmak":
        sub_frame.config(style="Highlighted.TFrame")
        name_label.config(foreground="blue")
    elif action == "kapatmak":
        sub_frame.config(style="Normal.TFrame")
        name_label.config(foreground="black")
    print(f"{name_label.cget('text')} {action} düğmesine tıklandı.")
...
style.configure("Highlighted.TFrame", background="blue")
...
for i, (path, name) in enumerate(profiles):
    ...
    name_label = tk.Label(sub_frame, text=name, foreground="black")
    name_label.pack()

    button_open = ttk.Button(sub_frame, text=f"{name} açmak", command=lambda sf=sub_frame, nl=name_label: on_button_click("açmak", sf, nl))
    button_open.pack(pady=5)

    button_close = ttk.Button(sub_frame, text=f"{name} kapatmak", command=lambda sf=sub_frame, nl=name_label: on_button_click("kapatmak", sf, nl))
